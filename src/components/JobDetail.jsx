...
import { parseReport, formatDate } from '../utils/parser';
...
export default function JobDetail({ jobId, navigate }) {
  const { jobs, updateJob, cvs, settings, activeCV } = useApp();
  const job = jobs.find((j) => j.id === jobId);

  const [jdOpen,         setJdOpen]         = useState(false);
  const [reanalyzeOpen,  setReanalyzeOpen]  = useState(false);
  const [reRunCvId,      setReRunCvId]      = useState(activeCV?.id || cvs[0]?.id || '');
  ...
  const handleReAnalyze = async () => {
    const cv = cvs.find((c) => c.id === reRunCvId);
    if (!cv) return;
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        ...
        {reanalyzeOpen && (
          <div className="px-6 pb-6 border-t border-slate-800 pt-5 space-y-4">
            <p className="text-xs text-slate-500">
              Change the CV or update your Settings, then re-run. The current report will be replaced.
            </p>

            {/* CV selector */}
            <div>
              <label className="block text-xs text-slate-400 mb-2">CV to use</label>
              {cvs.length === 0 ? (
                <p className="text-sm text-slate-500">No CVs saved. Add one in "My CVs".</p>
              ) : (
                <select
                  value={reRunCvId}
                  onChange={(e) => setReRunCvId(e.target.value)}
                  className="w-full bg-slate-800 border border-slate-700 text-slate-300 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-indigo-500"
                >
                  {cvs.map((cv) => (
                    <option key={cv.id} value={cv.id}>
                      {cv.name}{cv.id === activeCV?.id ? ' (Active)' : ''}
                    </option>
                  ))}
                </select>
              )}
